...
@IonicPage()
@Component({
  selector: 'page-add-salelist',
  templateUrl: 'add-salelist.html'
})
export class AddSaleListPage extends BasePage {
  ...
  ionViewDidEnter() {
    this.saleService.load().then(sale => {
      this.sale = sale;
  ...
  async onSubmit() {
    ...
    try {
      ...
      let salelist = this.prepareSaleListData();
      await salelist.save();
